        ...
        pub mod $mod_name {
            ...
            impl ExecTnum {
                            ...
                            self.has(c) ==> r.has(!c)
                {
                    ...
                    proof {
                        assert((!(0 as $uint)) & (0 as $uint) == (0 as $uint))
                            by(bit_vector);
                    }

                    let r = self.bw_xor(&ones);

                    proof {
                        assert forall|c: $uint| #![auto]
                            self.has(c) implies r.has(!c)
                        by {
                            assert(ones.has(!(0 as $uint)));
